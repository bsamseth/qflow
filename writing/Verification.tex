...
precision $\SI{64}{bit}$ floating point numbers.
...
by~\textcite{Jonsson-2018}. The results where obtained using $2^{23}$ MC
...
lot of samples change the positions of all particles substantially. For smaller
...
\begin{table}[h]
  \centering
  \caption{\label{tab:verify-energy-estimates}Estimated ground state energy
    using $\psi_G$ with two different values for $\alpha_G$. The energies are
    given per particle, and where produced using Importance sampling and
    $2^{23}$ samples. The errors given are corrected for autocorrelation using
    Blocking.}
...
The results are obtained using $\alpha_G=0.5$ and $2^{23}$ MC samples. For
...
\begin{table}[h]
  \centering
  \caption{\label{tab:verify-radius-estimates}Estimates for the mean radial
