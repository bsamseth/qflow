...
\subsubsection{Simple Non-Interacting Case}
...
potentials. The most commonly used is the simple Lennard-Jones
...
\end{align}
with $\epsilon = \SI{10.22}{\K\per\kappa}$ and $\sigma = \SI{2.556}{Å}$. We'll also use
the more accurate potential named HDFHE2~\cite{Kalos-1981}:
...
\begin{align}
    ...
    \left\{\!\begin{aligned}
        &A \exp(-\alpha r / r_m)\\
        &- F(r) \qty[ C_6  (r_m / r)^6 + C_8  (r_m / r)^8 + C_{10} (r_m / r)^{10}]
    \end{aligned}\right\}
\end{align}
with
