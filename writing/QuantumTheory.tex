...
\emph{solving} a classical system could be said to consist of the following steps:
...
time. The standard approach to solving this equation is through Separation of
Variables. We assume that the full wave function can be factorized as follows:
...
\end{align}
The solutions to this equation are known as the \emph{stationary states} of the system. If
...
Carlo integration. This will be discussed in more detail in \cref{chp:monte-carlo}.
...
With the natural units in place, the only involved quantity that has not been
given a unit is length, i.e. what unit does the $\vb r_i$ have. A convenient
...
1$.\footnote{Note that, due to the natural units, this implies that $\omega =
  1$, which further means that $a_{ho} = 1$. It should be apparent why these
  definitions are used, as it simplifies both units and expression quite a bit.} We do this because for
