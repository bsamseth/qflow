...
\acrlong{mci} as it is used here, followed the details of how to
...
For any finite $N$ the result of using \gls{mci} will not be exact. Because of this,
...
any difference could be entirely random. We would need to use more sample points
...
known as the \gls{sem}. It is common to use this
directly as the uncertainty of the expectation values.

This tells us that the expected statistical error we make when using \gls{mci} goes
...
\textcite{Jonsson-2018}. Say we have a sequence of sample value $f(x_i)$:
...
\end{figure}
The integrand is plotted in \cref{fig:mci-importance-example-func-plot}, and
...
\end{align}
Setting $g = \text{Uniform}(-5, 5)$ recovers \cref{eq:chp-mc-anon-2}, so this is simply the natural generalization of the standard approach for an arbitrary distribution function $g$.
...
\cref{fig:mci-importance-example-func-convergence} show the convergence of
...
listed in \cref{alg:inverse-transform-sampling}.
...
are interested in? The PDF in question is, as defined in \cref{eq:wavefunc-probability-density-def},
