...
While the algorithm it self is quite simple, the argument for \emph{why} it
...
\begin{algorithm}[h]
    ...
    \begin{algorithmic}[1]
        ...
        \Repeat
          ...
          \State $A\gets \flatfrac{P(\vx')q(\vx|\vx')}{P(\vx)q(\vx'|\vx)}$
          \If{$u \leq A$}
            \State $\vx \gets \vx'$
          \EndIf
          \State \Yield $\vx$
...
probabilities, $T(\vx'|\vx)$, describing the likelihood of moving between them.
The defining property of a Markov Chain, as opposed to other stochastic
processes is that the transition probabilities $T(\vx'|\vx)$ from a state $\vx$
...
constructing $T(\vx'|\vx)$ such that $\pi(\vx)=P(\vx)$, where $P(\vx)$ is the
desired probability distribution. 

In order to find the correct $T(\vx'|\vx)$, we require that the stationary
...
\end{description}
Deriving the correct $T(\vx'|\vx)$ starts by factoring the transition
probabilities into a proposal distribution, $q(\vx'|\vx)$, and an acceptance
ratio $A(\vx',\vx)$. The idea is to use the former to propose the next state,
and use the latter to accept or reject the proposal. Inserting this into
\cref{eq:detailed-balance} and rearranging we get:

\begin{align}
  \frac{A(\vx',\vx)}{A(\vx,\vx')} = \frac{P(\vx')}{P(\vx)}\frac{q(\vx|\vx')}{q(\vx'|\vx)}.
\end{align}
We can now freely choose an acceptance ratio that satisfies the above. The
common choice is:

\begin{align}
  A(\vx',\vx) = \min\qty(1, \frac{P(\vx')}{P(\vx)}\frac{q(\vx|\vx')}{q(\vx'|\vx)})
\end{align}

At this point, all that remains is to specify a proposal distribution
$\q(\vx'|\vx)$. Depending on the choice of $q$ we get a sampler with different
attributes. We have implemented two version, presented in the follwing sections.

\subsubsection{Metropolis Algorithm}

The simplest choice, referred to as simply the Metropolis algorithm, is to
choose a distribution such that $q(\vx'|\vx)=q(\vx|\vx')$. That way the
acceptance ratio simplifies significantly, fully canceling out $q$. In our
implementation we have used a uniform proposal distribution,
