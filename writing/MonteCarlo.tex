...
borderline meaningless, because we lack information on the precision of the
integrals. Assume that in reality, the numbers were
$\expval{H}_A=\SI{1.03(8)}{eV}$ and $\expval{H}_B=\SI{1.09(8)}{eV}$, where the
...
known as the \gls{sem}. It is common to one unit of this as the uncertainty on
the expectation values. Another common choice is to give confidence intervals
