...
This is simply a series of affine transformations, and as such could be
replaced with one single, compound transformation: $\vb{z}^{L} =
\mat{W}\vx + \vb{b}$. Turns out, without an activation function, no matter how many layers we
place in between the input and output, we have just defined a convoluted way of
...
$(-\infty, \infty)$ is possible. Finally, in our case, wave
functions tend to involve exponentials and so we shall make heavy use of the
exponential function as the output transformation on our networks.
...
need exponentially many nodes, but if allowed to be deeper, we can have
reasonable number of nodes as well~\cite{Hanin-2017}. A formal theorem for this
is not reproduced here, as to main point here is to stress the expressive
...
$\grad_{\valpha}\mathcal{C}(f(\vx))$, w.r.t. all model parameters $\valpha$.
...
where
\begin{align}
  \delta_k^{(L)}\defeq \pdv{\mathcal{C}}{z_k^{(L)}} = \pdv{\mathcal{C}}{a^{(L)}_{i}}\pdv{a_i^{(L)}}{z_k^{(L)}} = \pdv{\mathcal{C}}{a^{(L)}_{i}}\pdv{\sigma^{(L)}(z_k^{(L)})}{z_k^{(L)}}.
\end{align}
...
\end{align}
This way we can express the derivatives we need through $\vb{a}^{l}$ and
$\vb{\delta}^{(l)}$. We already know how to compute $\vb{a}^{l}$ for all layers,
so we just need $\vb{\delta^{l}}$ for $l < L$.
