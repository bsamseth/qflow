...
RBMSymmetricWavefunction::RBMSymmetricWavefunction(int M, int N, int f, Real sigma2, Real root_factor)
    : RBMWavefunction(M, N, sigma2, root_factor),
    _f(f)
{
...
Real RBMSymmetricWavefunction::deriv_w(int k, int l, System &system) const {
    Real res = 0;
    for (int i = k; i < _M; i += _f) {
        res += _root_factor / (1 + std::exp(-v_j(l, system))) * system.data()[i] / _sigma2;
    }
    return res;
}
