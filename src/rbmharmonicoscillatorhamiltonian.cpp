...
RBMHarmonicOscillatorHamiltonian::RBMHarmonicOscillatorHamiltonian(Real omega)
    : _omega(omega)
{
}
...
Real RBMHarmonicOscillatorHamiltonian::local_energy(System&       system,
                                                    Wavefunction& psi) const
{
    return external_potential(system) + internal_potential(system)
           - 0.5 * psi.laplacian(system);
}
