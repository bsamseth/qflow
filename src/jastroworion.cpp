...
Real JastrowOrion::drift_force(const System &system, int k, int d) {
    ...
    for (int i = 0; i < k; ++i) {
        ...
        res += (-square(beta) + std::abs(beta * gamma) / r_ik) * (system(k, d) - system(i, d));
    }
    for (int j = k+1; j < N; ++j) {
        const Real r_kj = distance(system, k, j);
        res += (-square(beta) + std::abs(beta * gamma) / r_kj) * (system(k, d) - system(j, d));
    }
    return 2 * res;
}
...
Real JastrowOrion::laplacian(System& system) {
    ...
    for (int k = 0; k < N; ++k) {
        ...
        for (int i = 0; i < k; ++i) {
            const Real r_ik = distance(system, i, k);
            term1 += (-square(beta) + std::abs(beta * gamma) / r_ik) * (system.row(k) - system.row(i));
            term2 += (-square(beta) + std::abs(beta * gamma) / r_ik) * D;
            term2 -= (system.row(i) - system.row(k)).squaredNorm() * std::abs(beta*gamma) / (r_ik * r_ik * r_ik);
        }
        for (int j = k+1; j < N; ++j) {
            const Real r_kj = distance(system, k, j);
            term1 += (-square(beta) + std::abs(beta * gamma) / r_kj) * (system.row(k) - system.row(j));
            term2 += (-square(beta) + std::abs(beta * gamma) / r_kj) * D;
            term2 -= (system.row(j) - system.row(k)).squaredNorm() * std::abs(beta*gamma) / (r_kj * r_kj * r_kj);
        }
