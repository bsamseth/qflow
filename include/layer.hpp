...
public:
    // Constructors simply forward to DenseLayer + initialized V and V_grad from W and W_grad.
    InputLayer(int inputs, int outputs, const activation::ActivationFunction& actFunc, Real scale_factor)
        : DenseLayer(inputs, outputs, actFunc, scale_factor),
          V(W.row(0)), V_grad(W_grad.row(0)) {}
    InputLayer(int inputs, int outputs, const activation::ReluActivation& actFunc)
        : DenseLayer(inputs, outputs, actFunc),
          V(W.row(0)), V_grad(W_grad.row(0)) {}
