...
void sampler_sanity(int sampler_type)
{
    ...
    for (int run = 1; run <= runs; ++run)
    {
        ...
        // ASSERT_DOUBLE_EQ will fail due to being 5 ULP different, where 4 ULP (unit in
        // the last place) is Google tests limit. This is still good enough.
        // ASSERT_FLOAT_EQ works.
        ASSERT_NEAR(init_system.rows() * init_system.cols() * 0.5, E_L, 1e-13);
    }
}

void sampler_integration_test(int  sampler_type,
                              Real expected_acceptance,
                              Real analytic_tol,
                              Real numeric_tol)
{
    ...
    EXPECT_NEAR(H.gross_pitaevskii_energy(init_system, psi), E_analytic, analytic_tol)
        << "ar = " << ar;
    EXPECT_NEAR(H.gross_pitaevskii_energy(init_system, psi), E_numeric, numeric_tol)
        << "ar = " << ar;
}
